...
exports.validateRow = function(data) {
	...
	catentrytype = data['Product Type'];
	...
	if (manufacturer !== undefined && partnumber !== undefined && catentrytype !== undefined) {
		if (manufacturer !== '' && partnumber !== '' && catentrytype !== '') {
			...
			console.log('Invalid row detected. Manufacturer, Manufacturer Part Number and Product Type is null.');
			return false;
		}
	}
	else {
		console.log('Invalid row detected. Manufacturer, Manufacturer Part Number and Product Type is undefined.');
		return false;
	}
};
...
function validateSequencingType(data, validation) {
	...
	var supportedSequencingTypes = dataHelper.splitStringToArray(applicationProperties.path().catman.transform.wsv.supportedSequencingTypes, ',');
...
function validateMasterCatalogCategory(data,lookups, validation) {
	...
	if(mastercatcategorylookup[data['Master Catalog']]  == undefined){
			...
			validation.errorMessages.push(data['Master Path'] + ' does not exist ');
			validation.isValid = false;
		}
	}
}
...
function validateSalesCatalogCategory(data,lookups, validation, jsonProperties) {
	...
	if (data['Sales Catalog']  !== undefined && data['Sales Catalog'].trim() !== ''){
		...
		for (var j = 0; j < salesCatalogDatAarray.length; j++)
		{
			...
			stCategories = storesCategories['WSV'];
...
function validateProductParentCat(data, validation) {
	if (data['Product Type'] ==='Product') {
		if (data['Master Catalog'] === '')
			{
			validation.errorMessages.push(' Master Path should not be Empty ');
			validation.isValid = false;
			}
	}
}
...
function validateSKU(data, validation){
	if(data['Product Type'] ==='SKU' && data['Parent'] ===''){
		if(data['Master Catalog'] === '')
		{
			validation.errorMessages.push(' Master Path should not be Empty ');
			validation.isValid = false;
...
function dataDrivenValidations(data, lookups, validation, productCreate) {
    ...
    var requiredForUpdate = ['Master Catalog','Manufacturer','Manufacturer Part Number','Product Type'];

    //console.log(inputfielddef['Name']);
    for (var k = 0; k < dataArray.length; k++) {

    	if (!dataArray[k].startsWith('WSV-'))
    		continue;
    	...
    	var inputFieldReqs = inputfielddef['WSV' + '-' + fieldName];
...
function attributeDictionaryValidation(data,lookups,validation) {
    ...
    var attrdef = lookups.header['WSV'];
	...
	// required fields for WSV since it has no attributes with minOccurence
