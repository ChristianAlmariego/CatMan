...
var env = process.argv[3]; // Environment argument
var storeName = process.argv[4]; // Store argument
...
var languageCategoryExport = [];
var languageCategoryPublish = [];
var languageItemExport = [];
var languageItemPublish = [];
var comments = [];
var fname= '';
var email = '';
var person = '';
...
controlFiles.forEach(function (controlFile){
	if (filesDict[controlFile] === 1) {
		...
		var catalogFileName = data[0].trim();
			...
			.on('end', function(controlFile){
				return function() {
					...
					if (validateControlFile(controlFile, controlFileData[controlFile])){
						...
						languageCategoryExport[catalogFileName] = {};
						languageCategoryPublish[catalogFileName] = {};
						languageItemExport[catalogFileName] = {};
						languageItemPublish[catalogFileName] = {};
						comments[catalogFileName] = {};
						...
						catalofFileReadStream.pipe(csvReadStream)
							.on('data', function(data){
								...
								try {
									...
									if(validation.isValid && category){
										console.log('Master Catalog Path: '+ data['Master Catalog Path'] + ' Saved!\n');
										writeToReportFile(controlFile,'Category: ' + data['Master Catalog Path'] + ' Saved!');
										validationRows[catalogFileName].rowsPassed++;
							      	}
							      	else if(validation.isValid){
							      		console.log('Item: ' + data['Code'] + ' Saved!\n');
							      		writeToReportFile(controlFile,'Item: ' + data['Code'] + ' Saved!');
							      		validationRows[catalogFileName].rowsPassed++;
							      	}
							      	else if(category){
									...
									if(validation.isValid){
										...
										if (code === "" && data['Master Catalog Path'] !== "") {
											if (data['Translation Status'] === 'Approved For Translation')
						    				{
						    					if (languageCategoryExport[catalogFileName][data['Translation Locale']] === undefined)
						    					{
						    						languageCategoryExport[catalogFileName][data['Translation Locale']] = [];
						    					}
						        				languageCategoryExport[catalogFileName][data['Translation Locale']][languageCategoryExport[catalogFileName][data['Translation Locale']].length] = data['Master Catalog Path'];
						    				}
						    				if (data['Translation Status'] === 'Approved For Publish')
						    				{
						    					if (languageCategoryPublish[catalogFileName][data['Translation Locale']] === undefined)
						    					{
						    						languageCategoryPublish[catalogFileName][data['Translation Locale']] = [];
						    					}
						    					languageCategoryPublish[catalogFileName][data['Translation Locale']][languageCategoryPublish[catalogFileName][data['Translation Locale']].length] = data['Master Catalog Path'];
						    				}
										}
										...
										if (code !== "" && data['Master Catalog Path'] === "") {
											if (data['Translation Status'] === 'Approved For Translation')
						    				{
						    					if (languageItemExport[catalogFileName][data['Translation Locale']] === undefined)
						    					{
						    						languageItemExport[catalogFileName][data['Translation Locale']] = [];
						    					}
						        				languageItemExport[catalogFileName][data['Translation Locale']][languageItemExport[catalogFileName][data['Translation Locale']].length] = data['Code'];
						    				}
						    				if (data['Translation Status'] === 'Approved For Publish')
						    				{
						    					if (languageItemPublish[catalogFileName][data['Translation Locale']] === undefined)
						    					{
						    						languageItemPublish[catalogFileName][data['Translation Locale']] = [];
						    					}
						    					languageItemPublish[catalogFileName][data['Translation Locale']][languageItemPublish[catalogFileName][data['Translation Locale']].length] = data['Code'];
						    				}
										}
											
											
											//EDS-4951 - Add comments field to XML sent to ULG
										var comment="";
										...
										if (code === "" && data['Master Catalog Path'] !== "") {
											if (comments[data['Master Catalog Path']] === undefined)
											{
												comments[data['Master Catalog Path']] = [];
											}
											comments[data['Master Catalog Path']] = comment;
										}
										else
										{
											if (comments[data['Code']] === undefined)
											{
												comments[data['Code']] = [];
											}
											comments[data['Code']] = comment;
										}
									}
							...
							.on('end', function(controlFile, catalogFileName){ return function() {
								...
								writeToReportFile(controlFile,'Number of rows successfully Saved: ' + validationRows[catalogFileName].rowsPassed);
								writeToReportFile(controlFile,'*************************');
								fname=catalogFileName;
								email=contactEmail;
								person=contactName;
								writeResult(controlFile, languageCategoryExport[catalogFileName], languageCategoryPublish[catalogFileName], 'category', comments);
								writeResult(controlFile, languageItemExport[catalogFileName], languageItemPublish[catalogFileName], 'item', comments);
								...
								if(contactEmail != "") {
									controlFileData[controlFile].emailSubject = isFailed ? ("TRANSLATION - " + env + " - " + catalogFileName +". Error Processing Items!") :
									("TRANSLATION - " + env + " - " + catalogFileName +". All Items were Successful!");
									var emailBody;
									...
									if (isFailed) {
										...
										emailBody = "Dear " + contactName + ", \n\nCatalog File: " + sharedFolderPath4Email + jsonObject.errorProcessingDirectory + catalogFileName + 
											...
											"\n\n**Note the business needs to only resubmit the rows that fail, and to not include requests that were successful**";
										sendValidationReportFile(reportDirectory + "Report-" + catalogFileName, errorProcessingDirectory + catalogFileName, contactEmail, '', controlFileData[controlFile].emailSubject, emailBody);
									} else {
										var sharedFolderPath4Email = jsonObject.baseSharedFolder4Email + translationsPath;
										emailBody = "Dear " + contactName + ", \n\nCatalog File: " + sharedFolderPath4Email + jsonObject.archiveDirectory + catalogFileName + 
											"\n\nReport File: " + sharedFolderPath4Email + jsonObject.reportDirectory + "Report-" + catalogFileName;
										sendValidationReportFile(reportDirectory + "Report-" + catalogFileName, archiveDirectory + catalogFileName, contactEmail, '', controlFileData[controlFile].emailSubject, emailBody);
									}
								}
	...
	baseDirControlFiles.forEach(function (controlFile){
		...
		if (filesDict[catalogFileName.trim()] != null && !(baseDirCatalogFiles.includes(catalogFileName))) {
			...
			if(controlFile.toString().toLowerCase().endsWith(".csv")){
				...
				if(catalogFileName.toString().toLowerCase().endsWith(".csv")){
					...
					if (contactEmail != null) {
						...
						sendValidationReportFile('', errorProcessingDirectory + catalogFileName, contactEmail, '' , emailSubject , "Dear " + contactName + ", \n\nControl File: " + errorProcessingDirectory + controlFile + 
						"\n\n" + "The File format is not supported")
					}
				}
				else{
			...
			if (contactEmail != null) {
				...
				sendValidationReportFile('', errorProcessingDirectory + controlFile, contactEmail, '' , emailSubject , "Dear " + contactName + ", \n\nControl File: " + errorProcessingDirectory + controlFile + 
				"\n\n" + "Catalog file referenced by more than one Control file");
			}
			if (duplicateControlContactEmail != "") {

				sendValidationReportFile('', errorProcessingDirectory + duplicateControlFileName, duplicateControlContactEmail, '' , emailSubject , emailSubject , "Dear " + duplicateControlContactName + ", \n\nControl File: " + errorProcessingDirectory + duplicateControlFileName + 
				"\n\n" + "Catalog file referenced by more than one Control file")
			}
		} else {
...
function writeResult(controlFile, languageExport, languagePublish, type, comments)
{
	var startresult = controlFile + "|" + fname + "|" + email + "|" + person + "|";
	var comment;
	var endresult;
	if (!isEmpty(languageExport))
	{
		fsWriteStream = fs.createWriteStream(baseDirectory+'ExtractForXLIFF/tms-export-control-' + type + batchid + controlFiles.indexOf(controlFile) + ".txt");
		for (var k in languageExport)
		{
			// implementation of batches to prevent sql query limitation
			var batchLimit = 300;
			var subBatchId = '';
			var referenceBatches = [];

			if (k !== undefined && k !== "") {
				referenceBatches = dataHelper.groupElementsIntoArrayOfArrays(languageExport[k], batchLimit);

				for (var batchKeyIndex in referenceBatches) {
					endresult = "|" + validLocales[k] + "|";
					comment = "|";

					for (var j = 0; j < referenceBatches[batchKeyIndex].length; j++) {
						if (j > 0) {
							endresult = endresult + ",";
							comment = comment + ",";
						}
						endresult = endresult + "''" + referenceBatches[batchKeyIndex][j] + "''";
						comment = comment + comments[referenceBatches[batchKeyIndex][j]];
					}
					
					// set new batchid per sub batches
					subBatchId = batchid + "-" + batchKeyIndex.toString() + "-";
					fsWriteStream.write(startresult + subBatchId + endresult + comment + os.EOL);
				}
			}
	...
	if (!isEmpty(languagePublish))
	{
		fsWriteStream = fs.createWriteStream(baseDirectory+'ExtractForPublish/tms-publish-control-' + type + batchid + controlFiles.indexOf(controlFile) + ".txt");
		for (var k in languagePublish)
		{
			if (k !== undefined && k !== "")
			{
				endresult = "|" + validLocales[k] + "|";
				for (var j = 0; j < languagePublish[k].length; j++)
				{
					if (j > 0)
					{
						endresult = endresult + ",";
					}
					endresult = endresult + "''" + languagePublish[k][j] + "''";
				}
...
{	
	...
	validateTranslationHeaders(data, validation);
	...
	else if(translateStatus !== "Approved For Translation" && status == "Translation")
	{
		console.log("@@@ERROR: Invalid Translation Status For Translation Request");
		validation.errorMessages.push("@@@ERROR: Invalid Translation Status For Translation Request");
		validation.isValid = false;
	}
	else if(translateStatus !== "Approved For Publish" && status == "Publish")
	{
		console.log("@@@ERROR: Invalid Translation Status For Publish Request");
		validation.errorMessages.push("@@@ERROR: Invalid Translation Status For Publish Request");
		validation.isValid = false;
	}
...
// check headers typos for translation
function validateTranslationHeaders(rowData, validation) {
	var supportedCatalogHeaders = [];
	supportedCatalogHeaders.push(constants.CSV_HEADER_SOURCELOCALE);
	supportedCatalogHeaders.push(constants.CSV_HEADER_TRANSLATIONLOCALE);
	supportedCatalogHeaders.push(constants.CSV_HEADER_TRANSLATIONSTATUS);
	supportedCatalogHeaders.push(constants.CSV_HEADER_CAT_MASTERCATALOGPATH);
	supportedCatalogHeaders.push(constants.CSV_HEADER_CODE);
