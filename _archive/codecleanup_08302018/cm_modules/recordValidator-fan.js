...
exports.getProductCode = function(data) {
	var partnumber = data['Primary Key'];
	return partnumber;
};
...
exports.validateRow = function(data) {
	...
	if (manufacturer !== undefined && partnumber !== undefined && catentrytype !== undefined) {
		if (manufacturer !== '' && partnumber !== '' && catentrytype !== '') {
			...
			console.log('Invalid row detected. Manufacturer, Manufacturer Part Number and Item Type is null.');
			return false;
		}
	}
	else {
		console.log('Invalid row detected. Manufacturer, Manufacturer Part Number and Item Type is undefined.');
		return false;
	}
};
...
function validateMasterCatalogCategory(data,lookups, validation) {
	...
	if(data['Master Path'] !== '' ){
	if(mastercatcategorylookup[data['Master Path']]  == undefined){
...
function validateSalesCatalogCategory(data,lookups, validation, jsonProperties) {
	...
	if (data['Sales Path']  !== undefined && data['Sales Path'].trim() !== ''){
		var salesCatalogDatAarray = data['Sales Path'].split("|");
		...
		for (var j = 0; j < salesCatalogDatAarray.length; j++)
		{
			...
			stCategories = storesCategories['FAN'];
...
function validateSKU(data, validation){
	if(data['Item Type'] ==='SKU' && data['Parent'] ===''){
		if(data['Master Path'] === '')
		{
			validation.errorMessages.push(' Master Path should not be Empty ');
			validation.isValid = false;
...
function dataDrivenValidations(data, lookups, validation, productCreate) {
    ...
    var requiredForUpdate = ['Master Path','Manufacturer','Manufacturer Part Number','Item Type'];

    //console.log(inputfielddef['Name']);
    for (var k = 0; k < dataArray.length; k++) {

    	if (!dataArray[k].startsWith('FAN-'))
    		continue;
    	...
    	var inputFieldReqs = inputfielddef['FAN' + '-' + fieldName];
...
function attributeDictionaryValidation(data,lookups,validation) {
    ...
    var attrdef = lookups.header['FAN'];
