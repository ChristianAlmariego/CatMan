...
exports.getOutputStreams = function(jsonObject,batchid) {
	...
	outputStreams.csvCatEntPriceStream.on('readable', function(){
		...
		{
			fsoutputStreams.csvCatEntPriceStream = fs.createWriteStream(outputDirectory + 'catentryprice-output.csv', {highWaterMark: Math.pow(2,14)});
		}
	...
	*/
	...
	outputStreams.csvCatEntDescThumbnailStream = stringify(writeoptions);
	outputStreams.csvCatEntDescThumbnailStream.on('readable', function(){
		if (fsoutputStreams.csvCatEntDescThumbnailStream === undefined)
		{
			fsoutputStreams.csvCatEntDescThumbnailStream = fs.createWriteStream(outputDirectory + 'catentrythumbnail-output' + batchid + '.csv', {highWaterMark: Math.pow(2,14)});
		}
		while((row = outputStreams.csvCatEntDescThumbnailStream.read()) !== null){
			fsoutputStreams.csvCatEntDescThumbnailStream.write(row);
		}
	});
	outputStreams.csvCatEntDescThumbnailStream.on('error', function(err){
		console.log(err.message);
	});
	outputStreams.csvCatEntDescThumbnailStream.on('finish', function(){
		fsoutputStreams.csvCatEntDescThumbnailStream.end();
	});
	outputStreams.csvCatEntDescThumbnailStream.write(['CatalogEntryDescription','','','','']);
	outputStreams.csvCatEntDescThumbnailStream.write(['PartNumber','Type','LanguageId','Thumbnail','Delete']);
	...
	outputStreams.csvCatEntParentCatGrpRelStream.write(['PartNumber','ParentGroupIdentifier','ParentStoreIdentifier','Sequence','Delete']);
