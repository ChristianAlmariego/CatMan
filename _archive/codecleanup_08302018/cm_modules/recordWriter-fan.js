...
function getCatalogEntryType(data)
{
	...
	{
		catalogEntryType = data['Item Type'];
	}
	else if (data['Item Type'] === 'SKU')
	{
...
function writeAttributes(data,outputStreams,lookups) {
	...
	var headercols = [];
	var headerlookup = lookups.header['FAN'];
...
//Write FAN Img URL
function writeFANImgUrls(data,outputStreams,lookups) {
	var fanimgurllookup = lookups.fanimgurllookup;
	var attroutput = [];
	var output = [];
	var partnumber = data['Primary Key'];
	if (partnumber !== undefined && partnumber !== '') {
		for (var i = 1; i < 7; i++) {
			if (data['Image URL'+ i]  !== undefined && data['Image URL'+ i]  !== '')
			{
				var attrName = fanimgurllookup['Image URL'+ i];
				attroutput[0] = attrName; // PartNumber
				attroutput[1] = -1;
				attroutput[2] = 'Attribute_Dictionary_Descriptive_Attributes_Image_URLs_Image_URLs' + '_' + attrName.replace(/\W/g,'_') + '_' + 
				partnumber.replace(/\W/g,'_') + '_ValueID';
				attroutput[3] = 0
				attroutput[4] = data['Image URL'+ i].trim();
				attroutput[5] = '1';
				attroutput[6] = '';
				attroutput[7] = '';
				attroutput[8] = '';
				...
				output[3] = 'Attribute_Dictionary_Descriptive_Attributes_All_EMR_Core_Attributes' + '_' + output[1].replace(/\W/g,'_') + '_' + 
							partnumber.replace(/\W/g,'_') + '_ValueID';
				output[4] = data['Image URL'+ i].trim();
...
function writeSalesCatalogPath(data,outputStreams,lookups) {
	...
	if ((data['Sales Path']  !== undefined) && (data['Sales Path'].trim() !== '')){
		var dataarray = data['Sales Path'].split("|");
		for (var j = 0; j < dataarray.length; j++)
		{
			if (dataarray[j] !== undefined && dataarray[j].trim() !== '')
			{
				...
				output[2] ='FAN'; //ParentStoreIdentifier
...
exports.write = function(data,outputStreams,lookups,jsonProperties,rRequest) {
	...
	if (rRequest === 'CatalogEntryComponent') {
		...
		writeFANImgUrls(data,outputStreams,lookups);
