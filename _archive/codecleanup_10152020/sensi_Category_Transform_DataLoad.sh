...
export DLOADOUTPUTLOC=/opt/websphere/catalogManager/dataload/SENSI/
...
process_name="Category Transform Dataload - SENSI"
...
#execute dataload
cd $WCBINDIR
$DATALOADCMD $CURRDIR/dataload/wc-dataload-category-sales.xml -Djava.security.egd=file:/dev/./urandom -DXmlValidation=false -DlogFilePath=$CURRDIR/wc-dataload-sensi-category$batchfile.log -DstoreIdentifier=Sensi -DcatalogIdentifier=SENSI_SALES_CATALOG -DenvPath=$ENVPATH -DoutputLocation=$DLOADOUTPUTLOC -Dbatchfile=$batchfile

# dataload errors reporting
cd $CURRDIR
echo "Read Dataload Logs for Reporting"
$dataload_log_reader $env "$process_name" "$batchfile" "$CURRDIR/wc-dataload-sensi-category$batchfile.log"
