...
export DLOADOUTPUTLOC=/opt/websphere/catalogManager/dataload/ISE/
...
process_name="Category Transform Dataload - ISE"

#execute cmcatgroup transform script
cd $CURRDIR
~/node.sh $CURRDIR/cmcatgroup.js $*
...
#execute dataload
cd $WCBINDIR
$DATALOADCMD $CURRDIR/dataload/wc-dataload-category-sales.xml -Djava.security.egd=file:/dev/./urandom -DXmlValidation=false -DlogFilePath=$CURRDIR/wc-dataload-ise-category$batchfile.log -DstoreIdentifier=InSinkErator -DcatalogIdentifier=INSINKERATOR_SALES_CATALOG -DenvPath=$ENVPATH -DoutputLocation=$DLOADOUTPUTLOC -Dbatchfile=$batchfile

# dataload errors reporting
cd $CURRDIR
echo "Read Dataload Logs for Reporting"
$dataload_log_reader $env "$process_name" "$batchfile" "$CURRDIR/wc-dataload-ise-category$batchfile.log"
