...
if [ "$1" = "dev" ] || [ "$1" = "stage" ] || [ "$1" = "prod" ] || [ "$1" = "local" ] ; then
  ...
  echo "Extract Master PartNumbers Lookup Error: Invalid Environment"
  exit 1
fi

if [ "$2" = "" ] ; then
  echo "Extract Master PartNumbers Lookup Errorr: Build ID is Required"
  exit 1
else
  export build_id=$2
fi
...
export output_filename=$($property_reader $env sys lookups.masterPartNumbers)

export DATAEXTRACT=$wcbin_dir"dataextract.sh"

echo "Starting Process - Extract Master PartNumbers Lookup"
...
docker exec ts-utils-job-$build_id /bin/bash -c "$DATAEXTRACT ${catman_container_dir}${dataextract_folder}wc-dataextract-partnumber.xml -Djava.security.egd=file:/dev/./urandom -DlangId=-1 -DXmlValidation=false -DlogFilePath=${catman_container_dir}${lookupcsv_folder}${tmp_folder}wc-dataextract-partnumber-${build_id}.log -DstoreIdentifier=EmersonCAS -DcatalogIdentifier=EmersonCAS -DenvPath=$wcenvpath_dir -DoutputLocation=${catman_container_dir}${lookupcsv_folder}${tmp_folder}"
