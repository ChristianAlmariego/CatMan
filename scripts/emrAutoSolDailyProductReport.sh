...
docker exec ts-utils-job-$build_id /bin/bash -c "$DATAEXTRACTCMD ${catman_container_dir}dataextract/wc-dataextract-emr-autosol-product.xml -Djava.security.egd=file:/dev/./urandom -DXmlValidation=false -DlogFilePath=${catman_container_dir}/wc-dataextract-AutoSol-product.xml.log -DstoreIdentifier=EmersonCAS -DcatalogIdentifier=EmersonCAS -DlangId=-1 -DenvPath=$wcenvpath_dir -DoutputLocation=$output_location/AutoSol_"

#need to be disabled when local testing
cp $output_location/AutoSol_catalog.csv $business_output_location/AutoSol_catalog.csv
